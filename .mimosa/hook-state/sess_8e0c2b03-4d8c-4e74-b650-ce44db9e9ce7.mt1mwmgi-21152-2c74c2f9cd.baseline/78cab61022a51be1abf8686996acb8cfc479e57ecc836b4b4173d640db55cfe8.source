...
class FeedPlayerState(
    ...
    companion object {
         ...
         * достаточно.
         *
         * Работает это так: наблюдаемо Compose в `applyChanges` вызывает
         * `onForgotten` уходящей страницы раньше `onRemembered` пришедшей, то есть
         * `PlayerPool.yield()` возвращает плеер в канал до того, как новая страница
         * начнёт его просить. Вдобавок `rememberPooledPlayer` зовёт `acquire()` из
         * `scope.launch`, то есть уже после кадра.
         *
         * Порядок вызовов внутри `applyChanges` — деталь реализации Compose, а не
         * контракт. Опираемся на прогон на устройстве: 20 свайпов подряд без
         * спиннера от исчерпания пула. Если порядок когда-нибудь изменится,
         * симптомом будет ровно спиннер на свайпе — поднимать ёмкость до четырёх
         * тогда и придётся, ценой памяти, ради снижения которой пул и вводился.
         */
