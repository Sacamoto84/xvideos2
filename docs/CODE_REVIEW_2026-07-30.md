...
> **Срез:** `71f93fa` · **Статус:** закрыт · **Индекс:** [все документы](README.md)
>
> Закрывался постепенно: пароль Luscious (п. 3) уехал в `SecureCredentialStore`
> в проходе 2, `MANAGE_EXTERNAL_STORAGE` (п. 6) удалён из манифеста, разделение
> на модули (п. 13) выполнено, тесты (п. 25) есть. Пометки «не исправлено» ниже
> по тексту отражают состояние на 30.07.2026, а не сегодняшнее.
