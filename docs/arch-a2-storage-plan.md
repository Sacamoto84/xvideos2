...
Выполнен 16.08.2026, ветка `arch/a1-a4`, один коммит `743f2d3` на все три
задачи.

Отклонений от плана нет. Формат данных на диске не менялся, поэтому ручная
проверка на устройстве (раздел «Финальная проверка») остаётся единственным
непокрытым шагом: регрессия на существующих коллекциях R автотестами не
проверяется.
