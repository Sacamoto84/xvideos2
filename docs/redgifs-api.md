...
`getExplorerNiches(order = …)` объявлен с параметром, но **единственный
вызывающий его не передаёт**: `R_Saved_NichesCaches` качает весь список
постранично с умолчанием `subscribers`, а `R_ScreenNichesTab.filterAndSortNiches`
сортирует уже загруженное на клиенте.

Отсюда два следствия. `NICHES_NAME_A_Z`/`NICHES_NAME_Z_A` дали бы `400` (`name`
у `/v2/niches` не принимается), но на провод не уходят. И вычисление `sort`
внутри `getExplorerNiches` бессмысленно вдвойне: сервер этот параметр
игнорирует — при `order=subscribers` и `order=posts` выдача с `sort=asc` и
`sort=desc` совпадает побайтово.

То есть параметр `order` у `getExplorerNiches` — мёртвый код вместе с веткой
`when` для `sort`.
