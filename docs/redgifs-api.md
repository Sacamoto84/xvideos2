...
### «All time» без своей ленты

`Order.TOP_ALLTIME` есть в меню ленты гифок, но метода под него нет:
`ItemTopPagingSource` уводит его в `else` и отдаёт `getTopThisWeek`. То есть
выбор «All time» без поиска показывает неделю. Значение `alltime` на устройстве
не проверялось, поэтому метод не заводился — по правилу выше ошибочный `order`
молча вернёт не ту выдачу, и проверить это можно только глазами.

Пока так, откат в `else` пишет предупреждение в лог, а `Order.nearestIn` —
подстановка сортировки при смене набора — никогда не приводит в `TOP_ALLTIME`
сама. Выбрать его вручную по-прежнему можно.
