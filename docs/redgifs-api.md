...
**У каждого адреса свой набор `order`.** Одинаковых нет ни у одной пары:

| адрес | принимает | кто зовёт |
|---|---|---|
| `/v2/gifs/search` | `top`, `top7`, `top28`, `latest`, `score`, `trending` | ленты и поиск гифок |
| `/v2/users/{name}/search` | `trending`, `oldest`, `latest`, `top`, `top7`, `top28` | профиль |
| `/v2/niches/{niche}/gifs` | `trending`, `oldest`, `latest`, `best`, `hot` **и `top`** | лента ниши |
| `/v2/niches` | `posts`, `subscribers` | список ниш |

Построчно по значениям `Order`:

| на проводе | гифки | профиль | лента ниши | где в приложении |
|---|---|---|---|---|
| `top` | ✅ | ✅ | ✅ | `TOP` |
| `latest` | ✅ | ✅ | ✅ | `LATEST` |
| `trending` | ✅ | ✅ | ✅ (умолчание) | `TRENDING` |
| `top7` | ✅ | ✅ | **400** | `TOP_WEEK` |
| `top28` | ✅ | ✅ | **400** | `TOP_MONTH`, `TOP28` |
| `score` | ✅ | **400** | **400** | `RELEVANT` — только поиск |
| `oldest` | **400** | ✅ | ✅ | `OLDEST` — только профиль |
| `subscribers`, `posts` | — | — | — | `NICHES_*`, только `/v2/niches` |
| `name` | — | — | — | `NICHES_NAME_*` — **400 на `/v2/niches`**, но на провод не уходит |
| `alltime` | **400** | — | — | было `TOP_ALLTIME`, **удалено** |
| `recent`, `best`, `new` | **400** | 200, игнорируется | `best` ✅ | были `RECENT`, `BEST`, **удалены** |
| `""` | **400** | — | — | `FORCE_TEMP` — на провод не уходит |

Профильный адрес ведёт себя иначе: неперечисленные `recent`/`best`/`new` он
...
**Список в тексте ошибки бывает неполным.** У `/v2/niches/{niche}/gifs`
сообщение перечисляет `trending, oldest, latest, best, hot`, но `top` тоже
принимается и даёт свою выдачу. Проверено сравнением списков id при контрольном
замере на устойчивость (три запроса подряд без `order` дают побайтово равный
ответ, так что различия не шум).

### Чего проверка не нашла — и это важно записать

Два места выглядели сломанными и оказались исправными.

**Профиль, `top28` и `trending`, пустая выдача.** У автора с 1133 гифками
`order=top28` вернул `total=0`. Это не поломка, а окно по времени:

| автор | последний пост | `top28` total |
|---|---|---|
| `ginger4play2` | 1 день назад | 299 |
| `bitchinbubba` | 259 дней назад | 0 |

«Топ за месяц» у автора, не постившего месяц, пуст по определению. В меню
профиля (`ScreenRedProfileSM.orderList`) `TOP28` и `TRENDING` остаются
законными — просто на неактивном авторе показывают пустой экран без пояснения.

**Лента ниши, `order=top`.** Его нет в списке из текста ошибки, но он
принимается и даёт выдачу, отличную от умолчания. Меню
(`NicheBottomBar`) — `TRENDING`, `TOP`, `LATEST` — работает целиком.

### Сортировка ниш идёт мимо сервера

`R_Saved_NichesCaches` качает весь список ниш постранично, а сортирует уже
загруженное `R_ScreenNichesTab.filterAndSortNiches` — на клиенте. К серверу
выбор из меню не уходит вообще.

До 06.08.2026 `getExplorerNiches` принимал `order: Order`, выводил из него
`sort` и подставлял оба в запрос. Ни один вызывающий этот параметр не передавал,
и оба довода за его удаление подтвердились проверкой:

- `/v2/niches` принимает всего два значения — `posts` и `subscribers`.
  `NICHES_NAME_A_Z`/`NICHES_NAME_Z_A` дают `name`, на который сервер отвечает
  `400 BadOrder`. То есть половина `Order`, которую параметр принимал по типу,
  сломала бы запрос;
- `sort` сервер игнорирует: при `order=subscribers` и `order=posts` выдача с
  `sort=asc` и `sort=desc` совпадает побайтово.

Параметр удалён, порядок зашит в путь (`order=subscribers&sort=desc`).
Проверено, что выдача не изменилась: страницы 1 и 2 до и после правки
совпадают по списку имён.

Сортировка ниш остаётся клиентской, и иначе быть не может: серверной сортировки
по имени у `/v2/niches` нет вовсе.
