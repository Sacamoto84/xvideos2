...
class SubscriptionsAndSavedLikesTest {
    ...
    @Test
    fun `profile back hierarchy prioritizes clearing tags then pop`() {
        fun resolveBackStep(hasTags: Boolean): String = when {
            hasTags -> "CLEAR_TAGS"
            else -> "POP"
        }

        assertEquals("CLEAR_TAGS", resolveBackStep(hasTags = true))
        assertEquals("POP", resolveBackStep(hasTags = false))
    }
}
